...
for product_id in product_ids:
    ...
    if product_id == 0x0102:
        ...
        dongle.ctrl_transfer(0x40, 0xFF, 0, 0, b'', timeout=usb_timeout)
    try:
        dongle.reset()
    except Exception:
        pass
...
# to 1000ms to allow time for USB to reinitialize after the CrazyRadio
# or RFStorm firmware jumps to the bootloader
logging.info("Looking for a device running the Nordic bootloader")
start = time.time()
while time.time() - start < 1:

    # Find a device running the Nordic bootloader
    try:
...
page_count = len(data) // 512  # Use integer division for page count
